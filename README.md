...
sequenceDiagram
  ...
  App->>App: Create PKCE verifier, challenge, and state
  App->>GoogleAuth: Redirect with code_challenge and state
  GoogleAuth->>App: Redirect to /auth/callback with code and state
  App->>App: Validate state and pending PKCE request
  App->>Server: Send code and PKCE verifier
...
- Pending PKCE verifier and state are stored in `sessionStorage` because they are only needed during the redirect flow.
